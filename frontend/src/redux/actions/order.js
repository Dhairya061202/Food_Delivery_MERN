...
export const paymentVerification=( 
    razorpay_payment_id,razorpay_order_id,razorpay_signature,orderOption) => async(dispatch)=>{
    try {
        ...
        const{data} = await axios.post(`${server}/paymentVertification`,{
            razorpay_payment_id,razorpay_order_id,razorpay_signature,orderOption
        },{
            headers:{
                "Content-Type":"application/json",
            },
...
export const getOrderDetails= (id)=>async(dispatch)=>{
    try {
        ...
        dispatch({type:"getOrderDetailsSuccess",
            payload:data.order
        });

    } catch (error) {
        dispatch({type:"getOrderDetailsFail",
            payload:error.response.data.message
        });
    }
}
